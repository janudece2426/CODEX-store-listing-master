...
export default function Filters({ activeTab, filters, managers, floors, onTabChange, onFilterChange, onReset }) {
  ...
  return (
    <section className="rounded-2xl border border-[#ead8d1] bg-white p-3 shadow-[0_8px_24px_rgba(74,45,54,0.06)] sm:p-4">
      <div className="flex flex-col gap-3 sm:gap-4">
          ...
          {['전체', '임대', '매매'].map((tab) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                activeTab === tab
                  ? 'bg-[#4a2d36] text-white shadow-[0_6px_14px_rgba(74,45,54,0.18)]'
                  : 'bg-[#fff4ef] text-[#5f4b52] hover:bg-[#f6e5dd]'
              }`}
            >
              {tab}
            </button>
        ...
        <div className="flex items-center gap-2 lg:hidden">
          <button
            ...
            className="flex h-11 flex-1 items-center justify-center rounded-xl border border-[#ead8d1] bg-[#fff4ef] text-sm font-bold text-[#4a3a3f]"
          >
          ...
          <button
            ...
            className="h-11 rounded-xl border border-[#ead8d1] bg-white px-4 text-sm font-bold text-[#5f4b52]"
          >
            초기화
          </button>
        </div>
        ...
        <div className={`${isMobileOpen ? 'grid' : 'hidden'} gap-3 md:grid-cols-2 lg:grid xl:grid-cols-6`}>
          <label className="text-xs font-bold text-[#6a5960]">
            광역지역
            <select
              ...
              className="mt-1 h-11 w-full rounded-xl border border-[#ead8d1] bg-white px-3 text-sm font-semibold text-[#3d3035] outline-none focus:border-[#b86562] focus:ring-4 focus:ring-[#b86562]/15"
            >
            ...
            const options =
              field.key === 'manager'
                ...
                : field.key === 'floor'
                  ...
                  : field.options;
            return (
              <label key={field.key} className="text-xs font-bold text-[#6a5960]">
                {field.label}
                <select
